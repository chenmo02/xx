...
- **打开文件**：支持加载本地 `.excalidraw` / `.json` 文件（模拟拖放方式加载，兼容 Excalidraw 新版）
- **保存文件**：将当前画布保存为 `.excalidraw` 文件
- **导出 PNG**：从画布直接导出为 PNG 图片
- **导出 SVG**：导出为矢量 SVG 格式
- **浏览器打开**：一键在默认浏览器中打开 Excalidraw
- 文件数据通过 Base64 编码传输，避免特殊字符转义问题
- 首次使用需要网络连接加载页面
...
### v1.5.1
- 🐛 修复 Excalidraw 画板 **打开本地文件一直转圈** 的问题
  - 原因：Excalidraw 新版改用 IndexedDB 存储，旧的 `localStorage + reload` 方式失效
  - 修复：改为模拟拖放（Drag & Drop）方式加载文件，使用 Excalidraw 原生文件处理 API
- ⚡ 文件数据传输改为 **Base64 编码**，彻底解决 JSON 特殊字符转义导致的加载失败
- ⚡ 新增 `LOAD:OK / LOAD:ERROR` 消息回传，加载状态实时反馈到状态栏
