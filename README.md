...
## Visual Studio 启动配置补充

- `CCToolbox (VS本地构建版)`：日常开发和调试使用，启动的是 `WpfApp1\bin\...` 下的本地构建产物
- `CCToolbox (单文件EXE，已发布)`：验证打包行为使用，启动的是 `artifacts\publish\win-x64-single\CCToolbox.exe`
- 使用第二个配置前，请先执行一次单文件发布命令，否则 Visual Studio 会因为找不到已发布的 EXE 而无法启动
