...
namespace WpfApp1.Services
{
    ...
    public static class JsonGridParser
    {
        ...
        private static JsonGridNode ParseElement(JsonElement element, string key)
        {
            ...
            switch (element.ValueKind)
            {
                case JsonValueKind.Object:
                    ...
                    foreach (var prop in element.EnumerateObject())
                        node.Children.Add(ParseElement(prop.Value, prop.Name));
                    break;
                ...
                case JsonValueKind.Array:
                    ...
                    if (IsObjectArray(element))
                    {
                        ...
                        BuildObjectArrayTable(element, node);
                    }
                    else
                    {
                        // 简单数组：构建为单列表格（直接显示值，不显示 [0],[1],[2]）
                        node.IsSimpleArray = true;
                        BuildSimpleArrayTable(element, node, key);
                    }
                    break;
        ...
        /// </summary>
        private static void BuildSimpleArrayTable(JsonElement array, JsonGridNode node, string key)
        {
            ...
            foreach (var item in array.EnumerateArray())
            {
                ...
                if (item.ValueKind is JsonValueKind.Object or JsonValueKind.Array)
                {
                    ...
                    cell.NestedChildren.Add(ParseElement(item, $"[{idx}]"));
                }
                else
                {
        ...
        /// 对象数组 → 多列表格
        /// </summary>
        private static void BuildObjectArrayTable(JsonElement array, JsonGridNode node)
        {
                ...
                foreach (var colName in colSet)
                {
                    ...
                    if (item.TryGetProperty(colName, out var val))
                    {
                        ...
                        if (val.ValueKind is JsonValueKind.Object or JsonValueKind.Array)
                        {
                            ...
                            cell.NestedChildren.Add(ParseElement(val, colName));
                        }
                        else
                        {
